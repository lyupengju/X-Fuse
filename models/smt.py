    ...
    def __init__(self, img_size=96, in_chans=1, num_classes=15, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
               ...
               ca_attention= ca_attentions[i], expand_ratio=expand_ratio)
...
def smt_t(pretrained=False, **kwargs):
    model = SMT(
        embed_dims=[64, 128, 256, 512], ca_num_heads=[4, 4, 4, -1], sa_num_heads=[-1, -1, 8, 16], mlp_ratios=[2, 2, 2, 2], 
...
if __name__ == '__main__':
    ...
    # net = SMT(img_size=96, in_chans=1, num_classes=2,
    #     embed_dims=[30*2, 60*2, 120*2, 240*2], ca_num_heads=[3, 3, 3, 3], sa_num_heads=[-1, -1, -1, 4], mlp_ratios=[2, 2, 2, 2], 
    #     qkv_bias=True, depths=[2, 2, 2, 2], ca_attentions=[1, 1, 1, 1], head_conv=3, expand_ratio=2).cuda()
    # model = net.cuda()
    ...
    with torch.no_grad():
        ...
        input = torch.rand(1, 1, 512,512, 300).cuda()
        net = SMT(img_size=96, in_chans=1, num_classes=2,
        embed_dims=[30*2, 60*2, 120*2, 240*2], ca_num_heads=[3, 3, 3, 3], sa_num_heads=[-1, -1, -1, 4], mlp_ratios=[2, 2, 2, 2], 
        qkv_bias=True, depths=[2, 2, 2, 2], ca_attentions=[1, 1, 1, 1], head_conv=3, expand_ratio=2).cuda()
        model = net.cuda()
